...
class MCTS(object):
    ...
    def tree_policy(self):
        ...
        return random.choice(
            [
                key
                for key in leaf_eval.keys()
                if leaf_eval[key] == max(leaf_eval.values())
            ]
        )
    ...
    def get_reward(self, sequence):
        ...
        sim_world = copy.copy(
            self.GP
        )  # TODO try selecting a simulated world from spectral sampling
        samples = []
        obs = []
        cost = 0
        reward = 0
        ...
        for s in samples:
            ...
            if self.f_rew == "mes" or self.f_rew == "maxs-mes":
                ...
                reward += self.aquisition_function(
                    time=self.t, xvals=xobs, robot_model=sim_world
                )
            ...
            try:
                if self.use_cost == False:
                    ...
                    if self.tree["child " + str(i)][2] == 0.0:
                        r = self.tree["child " + str(i)][3] / 100.0
                    else:
                        r = (
                            self.tree["child " + str(i)][3]
                            / self.tree["child " + str(i)][2]
                        )
                    value[i] = r
                if r > best:
                    best = r
...
class Tree(object):
    def __init__(self, f_rew, f_aqu, belief, pose, path_generator, t, depth, param, c):
        ...
        self.root = Node(
            pose, parent=None, name="root", action=None, dense_path=None, zvals=None
        )
        # self.build_action_children(self.root)

    def get_best_child(self):
        return self.root.children[
            np.argmax([node.nqueries for node in self.root.children])
        ]
        ...
        # At random node, after selected action from a specific node
            ...
            if self.f_rew == "mes" or self.f_rew == "maxs-mes":
                r = self.aquisition_function(
                    time=self.t, xvals=xobs, robot_model=belief, param=self.param
                )
            elif self.f_rew == "exp_improve":
                r = self.aquisition_function(
                    time=self.t, xvals=xobs, robot_model=belief, param=self.param
                ...
                r = self.aquisition_function(
                    time=self.t, xvals=xobs, robot_model=belief
                )
            ...
            if current_node.children is not None:
                ...
                if current_node.depth < self.max_depth - 1 and np.floor(
                    nchild**alpha
                ) == np.floor((nchild - 1) ** alpha):
            ...
            if True:
                if belief.model is None:
                    ...
                    zmean, zvar = (
                        np.zeros((n_points,)),
                        np.eye(n_points) * belief.variance,
                    )
                    zobs = np.random.multivariate_normal(mean=zmean, cov=zvar)
                    zobs = np.reshape(zobs, (n_points, 1))
                else:
    ...
    def get_next_child(self, current_node):
        ...
        return random.choice(
            [key for key in vals.keys() if vals[key] == max(vals.values())]
        )
...
class BeliefTree(Tree):
    def __init__(self, f_rew, f_aqu, belief, pose, path_generator, t, depth, param, c):
        super(BeliefTree, self).__init__(
            f_rew, f_aqu, belief, pose, path_generator, t, depth, param, c
        )
...
class cMCTS(MCTS):
    ...
    def choose_trajectory(self, t):
        ...
        # randomly sample the world for entropy search function
        if self.f_rew == "mes":
            self.max_val, self.max_locs, self.target = sample_max_vals(
                self.GP, t=t, visualize=True
            )
            param = (self.max_val, self.max_locs, self.target)
        elif self.f_rew == "exp_improve":
            param = [self.current_max]
            ...
            self.max_val, self.max_locs, self.target = sample_max_vals(
                self.GP, t=t, nK=int(self.aq_param[0]), visualize=True, f_rew=self.f_rew
            )
        ...
        print(
            [
                (node.nqueries, node.reward / node.nqueries)
        ...
        best_child = random.choice(
            [
                node
                for node in self.tree.root.children
                if node.nqueries == max([n.nqueries for n in self.tree.root.children])
            ]
        )
        all_vals = {}
        ...
        return (
            best_child.action,
            best_child.dense_path,
            best_child.reward / float(best_child.nqueries),
            paths,
            all_vals,
            self.max_locs,
            self.max_val,
