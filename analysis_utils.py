...
def predict_max(
    xvals, zvals, ranges=[0.0, 10.0, 0.0, 10.0], LEN=1.0, VAR=100.0, NOISE=0.5
):
...
def star_max_dist(
    ...
    try:
        ...
        np.savetxt(
            os.path.join(PATH, "sampled_maxes_dist.csv"),
    ...
    if SAVE_FLAG:
        np.savetxt(
            ...
            np.array([np.mean(dist_loc), np.mean(dist_val), entropy_x, entropy_z]),
        )
...
def make_df(file_names, column_names):
    ...
    if data.shape[1] > len(column_names):
        data = pd.read_table(
            d, delimiter=" ", header=None, skipfooter=data.shape[1] - len(column_names)
        )
        data = data.T
    ...
    for m in file_names[1:]:
        ...
        if temp_data.shape[1] > len(column_names):
            temp_data = pd.read_table(
                ...
                delimiter=" ",
                header=None,
                skipfooter=temp_data.shape[1] - len(column_names),
            )
            temp_data = temp_data.T
...
def make_samples_df(
    file_names, column_names, max_loc, max_val, xthresh=1.5, ythresh=2.50
):
    prop = []
    propy = []
    err_x = []
    ...
    sdata.loc[:, "YDistance"] = sdata.apply(
        lambda x: np.sqrt((x["z"] - max_val[0]) ** 2), axis=1
    )
    ...
    # For sim trials
    d_loc, d_val, entropy_x, entropy_z = star_max_dist(
        None, None, true_loc, true_val, PATH=os.path.split(file_names[0])[0]
    )
    # For car trials
    ...
    for i, m in enumerate(file_names[1:]):
        ...
        temp_data.loc[:, "YDistance"] = temp_data.apply(
            lambda x: np.sqrt((x["z"] - max_val[i + 1]) ** 2), axis=1
        )

        # Append proporations of samples within epsilon of x and delta of y
        prop.append(
            float(len(temp_data[temp_data.Distance <= xthresh])) / len(temp_data)
        )
        propy.append(
            float(len(temp_data[temp_data.YDistance <= ythresh])) / len(temp_data)
        )
        ...
        # For sim trials
        d_loc, d_val, entropy_x, entropy_z = star_max_dist(
            None, None, true_loc, true_val, PATH=os.path.split(m)[0]
        )
        # For car trials
...
def generate_stats(dfs, labels, params, end_time=149, fname="stats.txt"):
    ...
    for p in params:
        ...
        for df, label in zip(dfs, labels):
            ...
            f.write(
                label + " " + str(df_end[p].mean()) + ", " + str(df_end[p].std()) + "\n"
            )
    f.close()


def generate_histograms(
    dfs, props, labels, title, figname="", save_fig=False, ONLY_STATS=False
):
    ...
    for q, m in enumerate(props):
        print(
            labels[q]
            + " v PLUMES: "
            + str(stats.ttest_ind(props[0], m, equal_var=False))
        )
    ...
    for i in range(0, len(dfs)):
        if title == "200$m$ Budget Y Samples":
            hist[i], bins[i], _ = axes[i].hist(
                dfs[i]["YDistance"].values,
                bins=np.linspace(
                    min(dfs[0]["YDistance"].values),
                    max(dfs[0]["YDistance"].values),
                    np.floor(
                        max(dfs[0]["YDistance"].values)
                        - min(dfs[0]["YDistance"].values)
                    ),
            ...
            hist[i], bins[i], _ = axes[i].hist(
                dfs[i]["Distance"].values,
                bins=np.linspace(
                    min(dfs[0]["Distance"].values),
                    max(dfs[0]["Distance"].values),
                    np.floor(
                        max(dfs[0]["Distance"].values) - min(dfs[0]["Distance"].values)
                    ),
                ),
                color=colors[i],
                density=False,
...
def truncate_by_distance(
    ...
    temp_sidx = sample_df[
        np.isclose(sample_df["x"], last_samp_x)
        & np.isclose(sample_df["y"], last_samp_y)
    ].index
    ...
    if lawnmower == False:
        ...
        if len(temp_sidx) == 1:
            ...
            for i in temp_sidx:
                ...
                for j in range(1, i):
                    dist += np.sqrt(
                        (sample_df["x"].values[last] - sample_df["x"].values[j]) ** 2
                        + (sample_df["y"].values[last] - sample_df["y"].values[j]) ** 2
                    )
                    last = j
                if dist <= dist_lim:
                    candidates.append(i)
    ...
    d_loc, d_val, entropy_x, entropy_z = star_max_dist(
        xvals, zvals, max_loc, max_val[0], PATH=os.path.split(file_names[0])[0]
    )
    ...
    for p in params:
        ...
        for df, label, idx in zip(dfs, labels, ids):
            ...
            for j, i in enumerate(idx):
                ...
                temp_df = temp_df[i + 1 :]
            f.write(
                label + " " + str(np.mean(df_end)) + ", " + str(np.std(df_end)) + "\n"
            )
    f.close()


def distance_iteration_plots(
    dfs,
    trunids,
    labels,
    param,
    ...
    fig = plt.figure()
    colors = ["b", "g", "r", "c", "m", "y", "b", "g", "r", "c", "m", "y"]
    interp_granularity = float(
        dist_lim / granularity
    )  # uniform distance measures on which to interpolate the data for averaging
    ...
    for df, group_sidx in zip(dfs, trunids):
        ...
        for ids in group_sidx:
            ...
            temp = pd.concat(
                [d["distance"], d[param]], axis=1, keys=["distance", param]
            )
