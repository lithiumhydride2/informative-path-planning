...
class BlockWorld:
    ...
    def __init__(self, extent, num_blocks=1, dim_blocks=(2.0, 2.0), centers=None):
        """
        ...
        if centers is None:
            ...
            for i in range(0, num_blocks):
                self.centers.append(
                    (
                        (extent[1] - extent[0]) * np.random.random_sample() + extent[0],
                        (extent[3] - extent[2]) * np.random.random_sample() + extent[2],
                    )
                )
        else:
...
class BugTrap(BlockWorld):
    ...
    def __init__(
        ...
        if orientation == "left":
            points = [
                ...
                (
                    opening_location[0],
                    opening_location[1] + opening_size / 2 + channel_size,
                ),
                (
                    opening_location[0] + width + channel_size,
                    opening_location[1] + opening_size / 2 + channel_size,
                ),
                (
                    opening_location[0] + width + channel_size,
            ...
            self.inner_ylim = [
                opening_location[1] + opening_size / 2,
                opening_location[1] - opening_size / 2,
            ]
            self.outer_xlim = [
                opening_location[0],
    ...
    def in_obstacle(self, point, buff=0.1):
        if self.orientation == "left":
            if (
                point[0] > self.outer_xlim[0] - buff
                and point[0] < self.outer_xlim[1] + buff
            ):
                if (
                    point[1] < self.outer_ylim[0] + buff
                    and point[1] > self.outer_ylim[1] - buff
                ):
                    if (
                        point[0] >= self.inner_xlim[0] - buff
                        and point[0] <= self.inner_xlim[1] - buff
                    ):
                        if (
                            point[1] <= self.inner_ylim[0] - buff
                            and point[1] >= self.inner_ylim[1] + buff
                        ):
                            return False
                    return True
            return False
...
class ChannelWorld(BlockWorld):
    ...
    def __init__(self, extent, opening_location, opening_size, wall_thickness):
        ...
        top_wall = [
            (
                opening_location[0] - wall_thickness / 2,
                opening_location[1] + opening_size / 2,
            ),
...
if __name__ == "__main__":
    ...
    bt = BugTrap([0.0, 10.0, 0.0, 10.0], (3, 3), 2.0, 0.5, 2.0, "left")
    bt.draw_obstacles()
