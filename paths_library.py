    ...
    def buffered_paths(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            for c in fconfig:
                if (
                    c[0] > self.extent[0]
                    and c[0] < self.extent[1]
                    and c[1] > self.extent[2]
                    and c[1] < self.extent[3]
                    and not self.obstacle_world.in_obstacle((c[0], c[1]), buff=0.0)
                ):
                    ftemp.append(c)
                else:
                    break
            ...
            try:
                ...
                for m, c in enumerate(ttemp):
                    if (
                        c[0] <= self.extent[0] + 3 * self.tr
                        or c[0] >= self.extent[1] - 3 * self.tr
                        or c[1] <= self.extent[2] + 3 * self.tr
                        or c[1] >= self.extent[3] - 3 * self.tr
                        or self.obstacle_world.in_obstacle(
                            (c[0], c[1]), buff=3 * self.tr
                        )
                    ):
                        ttemp = ttemp[0 : m - 1]
    ...
    def make_sample_paths(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            coords[i] = [
                ...
                and not self.obstacle_world.in_obstacle(
                    (config[0], config[1]), buff=self.tr
                )
            ]
        ...
        for i, goal in enumerate(self.goals):
            ...
            if dist < self.step_size:
                ...
                for c in fconfig:
                    if (
                        c[0] > self.ranges[0]
                        and c[0] < self.ranges[1]
                        and c[1] > self.ranges[2]
                        and c[1] < self.ranges[3]
                        and not self.obstacle_world.in_obstacle(
                            (c[0], c[1]), buff=self.turning_radius
                        )
                    ):
                        ftemp.append(c)
                    else:
                ...
                try:
                    ...
                    for m, c in enumerate(ttemp):
                        if (
                            c[0] < self.ranges[0] + 3 * self.turning_radius
                            or c[0] > self.ranges[1] - 3 * self.turning_radius
                            or c[1] < self.ranges[2] + 3 * self.turning_radius
                            or c[1] > self.ranges[3] - 3 * self.turning_radius
                            or self.obstacle_world.in_obstacle(
...
class Reachable_Step_Generator(Reachable_Frontier_Generator):
    ...
    def take_step(self, loc):
        ...
        for i, goal in enumerate(self.goals):
            ...
            if dist > self.step_size:
                new_goal = (
                    loc[0] + self.step_size * np.sin(np.pi / 2 - angle_to_goal),
            ...
            for config in configurations:
                if (
                    config[0] > self.ranges[0]
                    and config[0] < self.ranges[1]
                    and config[1] > self.ranges[2]
                    and config[1] < self.ranges[3]
                    and not self.obstacle_world.in_obstacle(
                        (config[0], config[1]), buff=self.turning_radius
                    )
                ):
                    temp.append(config)
                else:
...
if __name__ == "__main__":
    ...
    gen = Reachable_Frontier_Generator(
        [0.0, 10.0, 0.0, 10.0], (20, 20), 0.5, 0.1, 1.5, bw
    )
