...
class GPModel:
    ...
    def __init__(
        self, ranges, lengthscale, variance, noise=0.0001, dimension=2, kernel="rbf"
    ):
        ...
        if kernel == "rbf":
            self.kern = GPy.kern.RBF(
                input_dim=self.dim, lengthscale=lengthscale, variance=variance
            )
        else:
    ...
    def predict_value(self, xvals, TEMP=False):
        ...
        if TEMP:
            ...
            mean, var = self.temp_model.predict(
                xvals, full_cov=False, include_likelihood=True
            )
            return mean, var
    ...
    def add_data_and_temp_model(self, xvals, zvals):
        ...
        self.temp_model = GPy.models.GPRegression(
            np.array(xvals), np.array(zvals), self.kern
        )
    ...
    def add_data(self, xvals, zvals):
        ...
        if self.model == None:
            self.model = GPy.models.GPRegression(
                np.array(self.xvals), np.array(self.zvals), self.kern
            )
        ...
        if model is not None:
            ...
            if visualize == True:
                ...
                plot = ax2.contourf(
                    ...
                    vmin=MIN_COLOR,
                    vmax=MAX_COLOR,
                    levels=np.linspace(MIN_COLOR, MAX_COLOR, 15),
                )
            ...
            while (
                maxima[0] < ranges[0]
                or maxima[0] > ranges[1]
                or maxima[1] < ranges[2]
                or maxima[1] > ranges[3]
            ):
                print(
                ...
                self.GP = GPModel(
                    ranges=ranges, lengthscale=lengthscale, variance=variance
                )

                # Take an initial sample in the GP prior, conditioned on no other data
                # This is done to
                xsamples = np.reshape(
                    np.array(data[0, :]), (1, dim)
                )  # dimension: 1 x 2
            ...
            print(
                "Environment initialized with bounds X1: (",
                self.x1min,
                ",",
                self.x1max,
                ")  X2:(",
            ...
            logger.info(
                "Environment initialized with bounds X1: ({}, {})  X2: ({}, {})".format(
                    self.x1min, self.x1max, self.x2min, self.x2max
                )
            )
...
class Path_Generator:
    def __init__(
        self, frontier_size, horizon_length, turning_radius, sample_step, extent
    ):
    ...
    def generate_frontier_points(self):
        ...
        angle = np.linspace(
            -2.35, 2.35, self.fs
        )  # fix the possibilities to 75% of the unit circle, ignoring points directly behind the vehicle
        goals = []
        for a in angle:
    ...
    def buffered_paths(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            for config in configurations:
                if (
                    config[0] > self.extent[0]
                    and config[0] < self.extent[1]
                    and config[1] > self.extent[2]
                    and config[1] < self.extent[3]
                ):
                    temp.append(config)
                else:
                    temp = []
...
class MCTS:
    ...
    def choose_trajectory(self, t):
        ...
        logger.info(
            "Number of rollouts: {} \t Size of tree: {}".format(i, len(self.tree))
        )

        paths = self.path_generator.get_path_set(self.cp)
        return (
            self.tree[best_sequence][0],
            best_val,
            paths,
            all_vals,
            self.max_locs,
    ...
    def initialize_tree(self):
        ...
        for action, samples in actions.items():
            cost = np.sqrt(
                (self.cp[0] - samples[-1][0]) ** 2 + (self.cp[1] - samples[-1][1]) ** 2
            )
            tree["child " + str(action)] = (
                samples,
                cost,
                0,
                0,
        ...
        for i, val in actions.items():
            try:
                ...
                leaf_eval[node] = self.tree[node][2] + 0.1 * np.sqrt(
                    2 * (np.log(self.tree["root"][1])) / self.tree[node][3]
                )
            except:
                pass
    ...
    def rollout_policy(self, node):
        ...
        for i in xrange(self.rl):
            actions = self.path_generator.get_path_set(
                self.tree[node][0][-1]
            )  # plan from the last point in the sample
            if len(actions) == 0:
    ...
    def get_reward(self, sequence):
        ...
        if self.f_rew == "mes" or self.f_rew == "maxs-mes":
            return self.aquisition_function(
                time=self.t,
                xvals=obs,
                robot_model=sim_world,
                param=(self.max_val, self.max_locs, self.target),
            )
        elif self.f_rew == "exp_improve":
            return self.aquisition_function(
                time=self.t, xvals=obs, robot_model=sim_world, param=[self.current_max]
            )
        else:
            return self.aquisition_function(
                time=self.t, xvals=obs, robot_model=sim_world
            )
...
class Robot(object):
    ...
    def __init__(
        ...
        self.GP = GPModel(
            ranges=extent, lengthscale=init_lengthscale, variance=init_variance
        )
        ...
        path_options = {
            "default": Path_Generator(
                frontier_size, horizon_length, turning_radius, sample_step, self.ranges
            ),
            "dubins": Dubins_Path_Generator(
                frontier_size, horizon_length, turning_radius, sample_step, self.ranges
            ),
            "equal_dubins": Dubins_EqualPath_Generator(
                frontier_size, horizon_length, turning_radius, sample_step, self.ranges
            ),
        }
    ...
    def choose_trajectory(self, t):
        ...
        for path, points in paths.items():
            ...
            value[path] = self.aquisition_function(
                time=t, xvals=points, robot_model=self.GP, param=param
            )
        try:
            return (
                paths[max(value, key=value.get)],
    ...
    def predict_max(self):
        ...
        if self.GP.xvals is None:
            return np.array([0.0, 0.0]).reshape(1, 2), 0.0
        ...
        for t in xrange(T):
            ...
            try:
                ...
                self.eval.update_metrics(
                    ...
                    max_loc=pred_loc,
                    max_val=pred_val,
                    params=[self.current_max, self.current_max_loc, max_val, max_locs],
                )
...
class Nonmyopic_Robot(Robot):
    ...
    def __init__(
        ...
        super(Nonmyopic_Robot, self).__init__(
            sample_world,
            start_loc,
            extent,
            kernel_file,
...
class Evaluation:
    ...
    def __init__(self, world, reward_function="mean"):
        ...
        logger.info(
            "World max value {} at location {}".format(self.max_val, self.max_loc)
        )
        ...
        if reward_function == "hotspot_info":
            ...
            raise ValueError(
                "Only 'mean' and 'hotspot_info' and 'info_gain' and ' mes' and 'exp_improve' reward functions currently supported."
            )
    ...
    def inst_regret(self, t, all_paths, selected_path, robot_model, param=None):
        ...
        for path, points in all_paths.items():
            if param is None:
                value_omni[path] = self.f_rew(
                    time=t, xvals=points, robot_model=robot_model
                )
            else:
                value_omni[path] = mves(
                    time=t,
                    xvals=points,
        ...
        if param is None:
            value_selected = self.f_rew(
                time=t, xvals=selected_path, robot_model=robot_model
            )
        else:
            value_selected = mves(
                time=t,
    ...
    def max_error(self, max_loc, max_val):
        ...
        Input:
            ...
            max_val (float)
        """
        return np.linalg.norm(max_loc - self.max_loc), np.linalg.norm(
    ...
    def hotspot_error(self, robot_model, NTEST=100, NHS=100):
        ...
        x1 = (
            np.random.random_sample((NTEST, 1)) * (self.world.x1max - self.world.x1min)
            + self.world.x1min
        )
        x2 = (
            np.random.random_sample((NTEST, 1)) * (self.world.x2max - self.world.x2min)
            + self.world.x2min
        )
    ...
    def update_metrics(
        ...
        (
            self.metrics["sample_regret_loc"][t],
            self.metrics["sample_regret_val"][t],
        ) = self.sample_regret(robot_model)
        (
            self.metrics["max_loc_error"][t],
            self.metrics["max_val_error"][t],
        ) = self.max_error(max_loc, max_val)

        self.metrics["instant_regret"][t] = self.inst_regret(
            t, all_paths, selected_path, robot_model
        )
        self.metrics["max_val_regret"][t] = self.inst_regret(
            t, all_paths, selected_path, robot_model, param="info_regret"
        )
        ...
        self.metrics["hotspot_error"][t] = self.hotspot_error(
            robot_model, NTEST=200, NHS=100
        )
    ...
    def plot_metrics(self):
        ...
        current_highest_obs_loc_x = np.array(
            self.metrics["current_highest_obs_loc_x"].values()
        )
        current_highest_obs_loc_y = np.array(
            self.metrics["current_highest_obs_loc_y"].values()
        )
        ...
        np.savetxt(
            "./figures/" + self.reward_function + "/metrics.csv",
            (
                time.T,
                info_gain.T,
                aqu_fun.T,
                ...
                regret.T,
                info_regret.T,
                current_highest_obs.T,
                current_highest_obs_loc_x.T,
...
def info_gain(time, xvals, robot_model, param=None):
    ...
    if xobs is None:
        ...
        entropy_after, sign_after = np.linalg.slogdet(
            np.eye(Sigma_after.shape[0], Sigma_after.shape[1])
    ...
    entropy_before, sign_before = np.linalg.slogdet(
        np.eye(Sigma_before.shape[0], Sigma_before.shape[1])
        + robot_model.variance * Sigma_before
    )

    # The term H(y_a, y_obs)
    entropy_after, sign_after = np.linalg.slogdet(
        np.eye(Sigma_total.shape[0], Sigma_total.shape[1])
        + robot_model.variance * Sigma_total
    )

    # The term H(y_a | f)
    entropy_total = (
        2 * np.pi * np.e * sign_after * entropy_after
        - 2 * np.pi * np.e * sign_before * entropy_before
    )
...
def hotspot_info_UCB(time, xvals, robot_model, param=None):
    ...
    return (
        info_gain(time, xvals, robot_model)
        + LAMBDA * np.sum(mu)
        + np.sqrt(beta_t) * np.sum(np.fabs(var))
    )
...
def sample_max_vals(robot_model, t, nK=3, nFeatures=300, visualize=True):
    ...
    for i in xrange(nK):
        ...
        W = np.random.normal(
            loc=0.0,
            scale=np.sqrt(1.0 / (robot_model.lengthscale**2.0)),
            size=(nFeatures, d),
        )
        b = 2 * np.pi * np.random.uniform(low=0.0, high=1.0, size=(nFeatures, 1))

        # Compute the features for xx
        Z = np.sqrt(2 * robot_model.variance / nFeatures) * np.cos(
            np.dot(W, robot_model.xvals.T) + b
        )
        ...
        if robot_model.xvals.shape[0] < nFeatures:
            ...
            try:
                Sigma = np.dot(Z.T, Z) + robot_model.noise * np.eye(
                    robot_model.xvals.shape[0]
                )
                ...
                theta = (
                    noise
                    - np.dot(Z, np.dot(U, R * (np.dot(U.T, np.dot(Z.T, noise)))))
                    + mu
                )
            except:
                ...
                logger.warning(
                    "[ERROR]: Sigma is not positive definite, ignoring simulation {}".format(
                        i
                    )
                )
...
def mves_maximal_set(time, xvals, robot_model, param):
    ...
    for i in xrange(max_vals.shape[0]):
        ...
        for ii, dist in enumerate(np.linspace(0.0, radius, radius_steps)):
            for jj, angle in enumerate(np.linspace(0.0, 2.0 * np.pi, angle_steps)):
                ball_data[ii * angle_steps + jj, :] = np.reshape(
                    max_locs[i]
                    + np.array([dist * np.cos(angle), dist * np.sin(angle)]),
                    (1, 2),
                )
...
def mves_maximal_set2(time, xvals, robot_model, param):
    ...
    for i in xrange(max_vals.shape[0]):
        ...
        # print("observations:", observations )
        mean_after, var_after = robot_model.predict_value(
            np.reshape(max_locs[i], (1, 2)), TEMP=True
        )
        # print("After mean var:", mean_after, var_after)
        ...
        # f += sum(entropy_of_tn(None, np.max(robot_model.zvals), mean, var) - entropy_of_tn(None, np.max([robot_model.xvals, np.max(observations)]), mean_after, var_after))
        f += (
            entropy_of_tn(a=max_before, b=None, mu=mean_before, var=var_before)
            - entropy_of_tn(a=max_after, b=None, mu=mean_after, var=var_after)
            + entropy_of_n(var)
        )
        # print("f:", f)
    # Average f
...
def mves(time, xvals, robot_model, param):
    ...
    for i in xrange(maxes.shape[0]):
        ...
        print("maxes:", maxes[i])
        """
        utility = entropy_of_n(var) - entropy_of_tn(
            a=None, b=maxes[i], mu=mean, var=var
        )
...
def entropy_of_tn(a, b, mu, var):
    ...
    return np.log(Z * np.sqrt(2.0 * np.pi * var)) + (
        alpha * phi_alpha - beta * phi_beta
    ) / (2.0 * Z)


def global_maximization(
    target,
    target_vector_n,
    target_grad,
    target_vector_gradient_n,
...
def exp_improvement(time, xvals, robot_model, param=None):
    ...
    avg_reward = (
        x * big_phi + np.sum(np.fabs(var)) * small_phi
    )  # (np.sum(mu)-eta)*big_phi + np.sum(np.fabs(var))*small_phi
