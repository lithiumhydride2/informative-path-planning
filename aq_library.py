...
def sample_max_vals(
    ...
    for i in xrange(nK):
        ...
        if robot_model.dimension == 2:
            W = np.random.normal(
                loc=0.0,
                scale=np.sqrt(1.0 / (robot_model.lengthscale)),
                size=(nFeatures, d),
            )
        elif robot_model.dimension == 3:
            W = np.random.normal(
                loc=0.0,
                scale=np.sqrt(1.0 / (robot_model.lengthscale[0])),
                size=(nFeatures, d),
            )
...
def global_maximization(
    ...
    if dim == 2:
        ...
        Xgrid_sample = np.vstack(
            [x1.ravel(), x2.ravel(), time * np.ones(len(x1.ravel()))]
        ).T
        Xgrid = Xgrid_sample
    ...
    if visualize:
        ...
        fig2.savefig(
            "./figures/" + str(f_rew) + "/opt/globalopt." + str(filename) + ".png"
        )
        # plt.show()
        plt.close()
        plt.close("all")
