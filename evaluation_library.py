...
class Evaluation:
    ...
    def __init__(self, world, reward_function="mean", num_stars=3):
        ...
        if world.dim == 2:
            ...
            self.max_loc = self.world.models[0].xvals[
                np.argmax(self.world.models[0].zvals), 0:-1
            ]
        ...
        if reward_function == "hotspot_info":
            ...
            raise ValueError(
                "Only 'mean' and 'hotspot_info' and 'info_gain' and ' mes' and 'maxs-mes' and 'exp_improve' reward functions currently supported."
            )
    ...
    def naive_reward(self, time, xvals, robot_model):
        """Predcited mean (true) reward function"""
        return aqlib.naive(
            time, xvals, robot_model, ((None, np.array(self.max_loc), None), 1.5)
        )

    def naive_value_reward(self, time, xvals, robot_model):
        """Predcited mean (true) reward function"""
        return aqlib.naive_value(
            time, xvals, robot_model, ((np.array(self.max_val), None, None), 3.0)
        )
    ...
    def sample_regret(self, robot_model):
        ...
        if robot_model.dimension == 2:
            ...
            avg_loc_dist = sp.spatial.distance.cdist(
                global_max_loc, robot_model.xvals[:, 0:-1]
            )
    ...
    def max_error(self, max_loc, max_val):
        ...
        return np.linalg.norm(max_loc[0:-1] - self.max_loc), np.linalg.norm(
    ...
    def update_metrics(
        ...
        if self.world.dim == 3:
            self.max_loc = self.world.models[t].xvals[
                np.argmax(self.world.models[t].zvals), 0:-1
            ]
        ...
        if self.reward_function == "naive" or self.reward_function == "naive_value":
            self.metrics["instant_regret"][t] = -1.0
            self.metrics["max_val_regret"][t] = -1.0
            self.metrics["mes_reward_robot"][t] = -1.0
            self.metrics["mes_reward_omni"][t] = -1.0
        else:
            self.metrics["instant_regret"][t], _, _ = self.inst_regret(
                t, all_paths, selected_path, robot_model
            )
            (
                self.metrics["max_val_regret"][t],
                self.metrics["mes_reward_robot"][t],
                self.metrics["mes_reward_omni"][t],
            ) = self.inst_regret(
        ...
        if params[2] is None:
            for i in range(0, self.num_stars):
                self.metrics["star_obs_" + str(i)][t] = -1.0
                self.metrics["star_obs_loc_x_" + str(i)][t] = -1.0
                self.metrics["star_obs_loc_y_" + str(i)][t] = -1.0
        else:
        ...
        self.metrics["hotspot_error"][t] = self.hotspot_error(
            t, robot_model, NTEST=200, NHS=100
        )
    ...
    def plot_metrics(self):
        ...
        mes_reward_robot = np.cumsum(
            np.array(self.metrics["mes_reward_robot"].values())
        )
        ...
        for i in range(0, self.num_stars):
            ...
            star_obs_loc_x.append(
                np.array(self.metrics["star_obs_loc_x_" + str(i)].values())
            )
            star_obs_loc_y.append(
                np.array(self.metrics["star_obs_loc_y_" + str(i)].values())
            )
