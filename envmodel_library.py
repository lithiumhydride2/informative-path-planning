    ...
    def __init__(
        ...
        if model is not None:
            ...
            if visualize == True:
                ...
                x1vals = np.arange(
                    ranges[0], ranges[1], (ranges[1] - ranges[0]) / NUM_PTS
                )
                x2vals = np.arange(
                    ranges[2], ranges[3], (ranges[3] - ranges[2]) / NUM_PTS
                )
                ...
                plot = ax2.contourf(
                    x1, x2, observations.reshape(x1.shape), 25, cmap="viridis"
                )
                ...
                ax2.scatter(
                    data[maxind, 0], data[maxind, 1], color="k", marker="*", s=500
                )
            ...
            for T in xrange(self.time_duration):
                ...
                while (
                    ...
                    or self.obstacle_world.in_obstacle(maxima, buff=0.0)
                ):
                    print(
                    ...
                    if T == 0:
                        ...
                        np.random.seed(seed)
                        observations = self.GP.posterior_samples(
                            data[1:, :], full_cov=True, size=1
                        )
