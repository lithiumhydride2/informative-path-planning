    ...
    for param, label, trial in zip(fileparams, labels, trials):
        ...
        for root, dirs, files in os.walk(path):
            for name in files:
                ...
                if "robot_model" in name and (
                    (trial in root and param in root and SUFFIX in root)
                    or ("lawnmower" in param and "lawnmower" in root)
                ):
                    ...
                    for s in seeds:
                        if s in root:
                            ...
                            # For Victoria
                                ...
                                max_loc.append(
                                    (
                                        float(ls[0].split(" ")[6].split("[")[1]),
                                        float(ls[0].split(" ")[7].split("]")[0]),
                                    )
                                )
