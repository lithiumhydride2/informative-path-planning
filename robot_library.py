...
class Robot(object):
    ...
    def choose_trajectory(self, t):
        ...
        try:
            best_key = np.random.choice(
                [key for key in value.keys() if value[key] == max(value.values())]
            )
            return (
                paths[best_key],
                true_paths[best_key],
    ...
    def predict_max(self, t=0):
        ...
        if self.dimension == 2:
            ...
            data = np.vstack(
                [x1.ravel(), x2.ravel(), self.time * np.ones(len(x1.ravel()))]
            ).T
    ...
    def visualize_reward(self, screen=False, filename="REWARD", t=0):
        ...
        reward = self.aquisition_function(
            time=t, xvals=data, robot_model=self.GP, param=param, FVECTOR=True
        )
        ...
        if not os.path.exists("./figures/" + str(self.f_rew)):
            os.makedirs("./figures/" + str(self.f_rew))
        fig2.savefig(
            "./figures/" + str(self.f_rew) + "/world_model." + str(filename) + ".png"
        )
        plt.close()
    ...
    def visualize_world_model(self, screen=True, filename="SUMMARY"):
        ...
        if self.MAX_COLOR is not None and self.MIN_COLOR is not None:
            ...
            if self.GP.xvals is not None:
                ...
                scatter = ax2.scatter(
                    self.GP.xvals[:, 0], self.GP.xvals[:, 1], c="k", s=10.0
                )
        else:
            plot = ax2.contourf(
                x1, x2, observations.reshape(x1.shape), 25, cmap="viridis"
